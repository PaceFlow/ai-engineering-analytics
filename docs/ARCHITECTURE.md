...
## Purpose
`ai-engineering-analytics` is a Rust CLI that turns local AI session history plus git history into a small set of productivity and quality metrics. The program has three major phases:
...
The entry point is [`src/main.rs`](/home/tadas/Work/paceflow/ai-engineering-analytics/src/main.rs). It parses CLI arguments and dispatches to one of four command handlers:
...
The ingest command ([`src/commands/ingest.rs`](/home/tadas/Work/paceflow/ai-engineering-analytics/src/commands/ingest.rs)) opens `~/.aea/aea.db`, loads all registered providers from [`src/providers/mod.rs`](/home/tadas/Work/paceflow/ai-engineering-analytics/src/providers/mod.rs), and processes each provider in turn.
...
### 2. Event storage
[`src/db/mod.rs`](/home/tadas/Work/paceflow/ai-engineering-analytics/src/db/mod.rs) creates the main `events` table and inserts provider events into it. While inserting, it enriches rows with:
...
After a provider finishes writing `events`, `ingest` immediately calls [`change_intel::pipeline::ingest_provider_code_changes`](/home/tadas/Work/paceflow/ai-engineering-analytics/src/change_intel/pipeline.rs). This is the second normalization layer.
...
## Core Data Model
The main tables are defined across [`src/db/mod.rs`](/home/tadas/Work/paceflow/ai-engineering-analytics/src/db/mod.rs) and [`src/change_intel/schema.rs`](/home/tadas/Work/paceflow/ai-engineering-analytics/src/change_intel/schema.rs).
...
[`src/commands/associate_commits.rs`](/home/tadas/Work/paceflow/ai-engineering-analytics/src/commands/associate_commits.rs) calls [`change_intel::commit_assoc::run`](/home/tadas/Work/paceflow/ai-engineering-analytics/src/change_intel/commit_assoc/mod.rs).
...
`compute_quality_metrics` in [`src/metrics/quality.rs`](/home/tadas/Work/paceflow/ai-engineering-analytics/src/metrics/quality.rs) derives:

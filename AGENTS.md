...
`src/` contains the entire CLI application. `src/main.rs` wires the `vca` binary and dispatches subcommands defined in `src/cli.rs`. Command handlers live in `src/commands/`, provider-specific ingestion logic in `src/providers/` and `src/change_intel/providers/`, shared analytics code in `src/metrics/`, and SQLite access in `src/db/`. Build artifacts are written to `target/` and should not be committed.
...
- `cargo run -- ingest` ingests local session data into the SQLite store.
- `cargo run -- associate-commits --repo /absolute/path/to/repo` links AI-attributed changes to git commits.
...
## Testing Guidelines
Tests are inline unit tests under `#[cfg(test)]` blocks in the owning module; there is no separate `tests/` directory yet. Add focused tests next to the code you change, especially around parser behavior, ingestion idempotency, and commit association logic. Use descriptive test names such as `parses_single_update_file` or `ingest_is_idempotent_with_cursor_skip`. Run `cargo test` before submitting changes.
...
This project analyzes local editor/session history and git metadata. Use absolute repo paths for `associate-commits`, avoid committing local databases or generated reports, and document any new provider-specific assumptions in `README.md`.

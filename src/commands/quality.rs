...
fn render_quality_report(
    ...
    if rows.is_empty() {
        if show_branch_hint {
            out.push_str(
                "No ticket-style task rows matched. Try `vca quality --group-by branch` or `--overall`.\n",
            );
        } else if repo_auto_injected {
            out.push_str(
                "No quality rows found for the current repo. Run `vca ingest` first, or pass `--all-projects` to include data from other ingested repos.\n",
            );
        } else {
            out.push_str("No quality rows found. Run `vca ingest` first.\n");
        }
        return out;
    }
...
mod tests {
    ...
    fn render_quality_report_suggests_branch_view_for_hidden_task_rows() {
        ...
        assert!(rendered.contains("`vca quality --group-by branch`"));
